...
// waits for readiness, and reports the connection env var + DSN file per
// dependency. Per-dependency failures are surfaced and cause a non-zero exit, but
// the daemon leaves nothing half-provisioned that blocks a retry (FR-009).
func provisionDependencies(c *client.Client, service string, deps []config.Dependency) error {
	...
	results, err := c.ApplyDependencies(context.Background(), service, reqs)
	if err != nil {
