...
// a private Infoblox-CTO CLI preset — can conform):
//
...
// through the SAME template data as the base scaffold (Name/Module/AppName/etc.),
// and written to <project>/<path>, overriding any base file already at that
// path.
//
// The public devedge CLI ships NO proprietary preset. A product-specific preset
// (concrete OIDC issuer/client, branding, extra commands) is provided by a
// private repo and is applied with --preset-dir. This keeps zero proprietary
// content in the public repo while leaving a clean, coherent seam.
...
// devedge CLI ships NONE (only a contract README under presets/), so this
// returns nil; it exists so the --preset-dir seam has a documented, testable
// home and mirrors internal/ufescaffold.
