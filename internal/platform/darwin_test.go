...
// TestPlist_ContainsDaemonEnv verifies the plist carries PATH, HOME, and
// KUBECONFIG in EnvironmentVariables, so the daemon does not run with the
// minimal launchd PATH (issue #9).
func TestPlist_ContainsDaemonEnv(t *testing.T) {
	data := plistData{
...
func TestInvokerHome_SudoUserWithoutHomeDir_FallsBack(t *testing.T) {
	// A SUDO_USER whose /Users/<name> does not exist must not be trusted.
