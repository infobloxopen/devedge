...
func (f *fakeProvisioner) EnsureInstance(_ context.Context, engine Engine, _ string) (Instance, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	if err := f.failInstance[engine]; err != nil {
		return Instance{}, err
	}
	f.instances[engine]++
	return Instance{Engine: engine, Host: string(engine) + "." + f.host, Port: f.port[engine]}, nil
}

func (f *fakeProvisioner) Ready(_ context.Context, engine Engine) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	if f.neverReady[engine] {
		return fmt.Errorf("connection refused")
	}
	f.readyCalls[engine]++
	if f.readyCalls[engine] < f.readyAfter[engine] {
